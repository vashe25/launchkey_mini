function remote_init(manufacturer, model)
	...
	local items={
		...
		{name="patch_name", output="text"},
		{name="group_name", output="text"},
		{name="device_name", output="text"},
		{name="track_name", output="text"},
	}
	...
	local inputs={
		...
		{pattern="b0 0d ?<???x>", name="Intelligent", port=1},
	}
	...
	local outputs={
		...
		{name="Cliplaunch 1", pattern="90 68 0<00xx>", x="value*3"},
		{name="Cliplaunch 2", pattern="90 78 0<00xx>", x="value*3"},
	}
...
function remote_probe(manufacturer, model, prober)
	...
	if model == "Launchkey Mini InControl" then
	...
	if received_ports[1] ~= nil then
		...
		local one_result = {in_ports = {port_out}, out_ports = {port_out}}
